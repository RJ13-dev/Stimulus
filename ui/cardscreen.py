...
class PuzzleCard(QWidget):
    def __init__(self, title, text, puzzle_id, parent=None):
        ...
        # Deep shadow to elevate the dark card from the dark wood background
        ...
        # Bright Ivory Title for perfect contrast on dark glass
        ...
        # Standout Solid Gold Button to pop against the dark frame
    ...
    def paintEvent(self, event):
        ...
        # Warm Smoked Glass Gradient
        grad = QLinearGradient(0, rect.top(), 0, rect.bottom())
        grad.setColorAt(0.0, QColor(45, 35, 30, 215))   # Dark warm charcoal, premium thickness
        grad.setColorAt(1.0, QColor(25, 20, 15, 185))   # Deep chocolate brown floor tone
        
        path = QPainterPath()
        path.addRoundedRect(rect, radius, radius)
        painter.fillPath(path, QBrush(grad))
        
        # Elegant outer wood-toned border
        painter.setPen(QPen(QColor(110, 85, 65, 120), 2))
        painter.drawPath(path)
        
        # Subtle internal frosted-glass highlight along the top edge
...
class CardScreenApp(QWidget):
    ...
    def show_puzzle(self, puzzle_id):
        """Hand off to the game controller or launch direct stand-alone windows."""
